...
const LOG_PREFIX = "[SettingsView]";

export function SettingsView({ onBack }) {
  const { user } = useAuth();
  const { showToast } = useToast();
  const { embeddingModel, setEmbeddingModel, EMBEDDING_MODELS } = useSettings();
  const {
    preference: htmlPreference,
    isLoading: isLoadingHtmlPreference,
    isSaving: isSavingHtmlPreference,
    setPreference: setHtmlPreference,
  } = useHtmlSelectionPreference(user?.uuid);

  const handleToggleHtmlPreference = useCallback(async () => {
    if (!user?.uuid) return;
    const nextValue = !Boolean(htmlPreference);
    try {
      await setHtmlPreference(nextValue);
      showToast({
        type: "success",
        description: nextValue
          ? "Using cleaned HTML by default."
          : "Using original HTML by default.",
      });
    } catch (error) {
      console.error(`${LOG_PREFIX} Failed to update HTML preference`, error);
      showToast({
        type: "error",
        description: "Unable to update HTML preference. Please try again.",
      });
    }
  }, [htmlPreference, setHtmlPreference, showToast, user?.uuid]);
  ...
  const handleModelChange = useCallback(
    (event) => {
      ...
      try {
        ...
        const isOpenAI = newModel === EMBEDDING_MODELS.OPENAI;

        console.log(`${LOG_PREFIX} Embedding model updated`, {
          model: newModel,
          is_openai: isOpenAI,
        });
        ...
        showToast({
          type: "success",
          description: `Embedding model set to ${
            isOpenAI ? "OpenAI" : "Paraphrase Multilingual"
          }`,
        });
      } catch (error) {
        console.error(`${LOG_PREFIX} Failed to update embedding model`, error);
        showToast({
          type: "error",
          description: "Unable to update embedding model. Please try again.",
        });
      }
    },
    [setEmbeddingModel, EMBEDDING_MODELS, showToast]
  );
      ...
      <div className="min-vh">
        <section className="form-section">
          <div className="extension-agents">
            ...
            {/* Use cleaned HTML automatically toggle */}
            <div className="html-preference-card">
              <div className="html-preference-card__copy">
                <p className="html-preference-card__title">
                  Use cleaned HTML automatically
                </p>
              </div>
              <label
                className={`toggle-switch ${htmlPreference ? "is-on" : ""} ${
                  isSavingHtmlPreference ? "is-disabled" : ""
                }`}
              >
                <input
                  type="checkbox"
                  checked={Boolean(htmlPreference)}
                  onChange={handleToggleHtmlPreference}
                  disabled={isSavingHtmlPreference || isLoadingHtmlPreference}
                />
            ...
            <div className="settings-dropdown-card">
              <label htmlFor="embedding-model" className="dropdown-label">
                Embedding Model
              </label>
              <select
                id="embedding-model"
                ...
                className="embedding-dropdown"
              >
                <option value={EMBEDDING_MODELS.OPENAI}>
                  OpenAI (text-embedding-3-large)
                </option>
                <option value={EMBEDDING_MODELS.PARAPHRASE}>
