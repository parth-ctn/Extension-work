...
import { EVENT_NAMES } from "../../constants/events.js";
...
const AGENT_SELECTION_EVENT = EVENT_NAMES.AGENT_SELECTION_CHANGED;
const LOG_PREFIX = "[LoggedInView]";
...
export function LoggedInView() {
  ...
  const persistSelectedAgent = useCallback(
    async (extensionId, { reason = "manual", agentOverride = null } = {}) => {
      const normalizedId =
      ...
      const agentData =
        ...
        agentsRef.current.find(
          (agent) => agent.extension_id === normalizedId
        ) ??
        null;

      setSelectedAgent(normalizedId);

      console.log(`${LOG_PREFIX} Persisting selected agent`, {
        extensionId: normalizedId || null,
        reason,
        hasAgentData: Boolean(agentData),
      });
      ...
      try {
        ...
        console.error(
          `${LOG_PREFIX} Failed to store selected extension ID in extension storage`,
          error
        );
      }

      setJSON(STORAGE_KEYS.SELECTED_EXTENSION_ID, normalizedId || undefined);
      setJSON(STORAGE_KEYS.SELECTED_AGENT_DATA, agentData ?? undefined);

      if (typeof window !== "undefined") {
        try {
          window.dispatchEvent(
            new CustomEvent(AGENT_SELECTION_EVENT, {
              detail: {
                extensionId: normalizedId || null,
                agent: agentData,
                reason,
          ...
          console.warn(
            `${LOG_PREFIX} Failed to dispatch agent change event`,
            error
          );
        }
      }
  ...
  // Fetch extension agents on mount
  useEffect(() => {
    const fetchAgents = async () => {
      ...
      try {
        const response = await extensionAgentService.getExtensionAgents({
          user_uuid: user.uuid,
        });

        // The API returns an array of agents directly
        const agentsList = Array.isArray(response) ? response : [];
        setAgents(agentsList);

        // Load previously selected agent from storage
        let storedExtensionId = null;
        try {
          storedExtensionId = await readExtensionStorage(
            STORAGE_KEYS.SELECTED_EXTENSION_ID
          );
        } catch (error) {
          console.warn(
            `${LOG_PREFIX} Failed to read selected extension ID`,
            error
          );
        }
        ...
        const hydratedAgent =
          storedExtensionId &&
          agentsList.find((agent) => agent.extension_id === storedExtensionId);

        if (hydratedAgent) {
          await persistSelectedAgent(storedExtensionId, {
            reason: "hydrate:stored",
            agentOverride: hydratedAgent,
          });
        } else if (agentsList.length > 0) {
          await persistSelectedAgent(agentsList[0].extension_id, {
            reason: "hydrate:default",
            agentOverride: agentsList[0],
          });
        } else {
          await persistSelectedAgent("", { reason: "hydrate:none" });
        }
      } catch (error) {
        console.error("Failed to fetch extension agents:", error);
        showToast({
          type: "error",
          description: "Failed to load extension agents",
        });
      } finally {
    ...
    fetchAgents();
  }, [user?.uuid, showToast, persistSelectedAgent]);

  // Handle agent selection change
  const handleAgentChange = useCallback(
    (extensionId) => {
      persistSelectedAgent(extensionId, {
        reason: "manual-selection",
        agentOverride:
          agents.find((agent) => agent.extension_id === extensionId) ?? null,
      }).catch((error) => {
        console.error(
          `${LOG_PREFIX} Failed to persist manual agent selection`,
          error
        );
      });
    },
    [agents, persistSelectedAgent]
  );
  ...
  const handleLogout = async () => {
    ...
    try {
      await logout();
      showToast({ type: "success", description: TOAST_MESSAGES.LOGGED_OUT });
    } catch (error) {
      console.error(error);
    ...
    <section className="main-section">
      <header className="section-header">
        ...
        <div className="user-action">
          ...
          >
            ...
            >
              <path
                d="M7.7501 7.374C7.7501 7.7882 8.0859 8.124 8.5001 8.124C8.9143 8.124 9.2501 7.7882 9.2501 7.374H7.7501ZM9.2501 1.5C9.2501 1.08579 8.9143 0.75 8.5001 0.75C8.0859 0.75 7.7501 1.08579 7.7501 1.5H9.2501ZM5.81092 4.16156C6.1879 3.98991 6.3543 3.54515 6.1827 3.16818C6.011 2.79121 5.56625 2.62478 5.18928 2.79644L5.81092 4.16156ZM1.5001 9.66L0.75 9.6561L0.75015 9.6687L1.5001 9.66ZM8.5001 16.5L8.5087 15.75C8.503 15.75 8.4972 15.75 8.4915 15.75L8.5001 16.5ZM15.5001 9.66L16.2502 9.6687L16.2501 9.6561L15.5001 9.66ZM11.8109 2.79644C11.4339 2.62478 10.9892 2.79121 10.8175 3.16818C10.6459 3.54515 10.8123 3.98991 11.1893 4.16156L11.8109 2.79644ZM9.2501 7.374V1.5H7.7501V7.374H9.2501ZM5.18928 2.79644C2.49853 4.02171 0.76549 6.69951 0.75 9.6561L2.25009 9.6639C2.26252 7.29237 3.65256 5.1444 5.81092 4.16156L5.18928 2.79644ZM0.75015 9.6687C0.79945 13.9046 4.27288 17.2986 8.5087 17.25L8.4915 15.75C5.08394 15.7892 2.28971 13.0588 2.25005 9.6513L0.75015 9.6687ZM8.4915 17.25C12.7273 17.2986 16.201 13.9046 16.2502 9.6687L14.7502 9.6513C14.7105 13.0588 11.9163 15.7892 8.5087 15.75L8.4915 17.25ZM16.2501 9.6561C16.2346 6.69951 14.5017 4.02171 11.8109 2.79644L11.1893 4.16156C13.3476 5.1444 14.7377 7.29237 14.7501 9.6639L16.2501 9.6561Z"
                fill="white"
              />
            </svg>
          </div>
      ...
      <div className="min-vh">
        <section className="form-section">
          <div className="extension-agents">
            <h3>Web Extension Agents</h3>
            {isLoadingAgents ? (
              <p
              ...
              <p
                style={{ textAlign: "center", color: "#666", padding: "20px" }}
              >
                No extension agents found
              </p>
            ) : (
              <ul className="extension-list">
                {agents.map((agent) => (
                  <li key={agent.extension_id}>
                    <label>
                      <input
                        type="radio"
                        name="extension"
                        value={agent.extension_id}
                        checked={selectedAgent === agent.extension_id}
                        onChange={(e) => handleAgentChange(e.target.value)}
                      />
                      <span>{agent.extension_agent_name}</span>
                    </label>
                  </li>
                ))}
              </ul>
