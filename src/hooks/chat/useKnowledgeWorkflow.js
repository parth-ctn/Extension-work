...
function normalizeWsToken(token) {
...
export function useKnowledgeWorkflow({
  ...
  useEffect(() => {
    const nextId = propSelectedExtensionIdRef.current;
    const currentId = stateRef.current?.context?.selectedExtensionId ?? null;
    if (nextId !== currentId) {
      updateContext({ selectedExtensionId: nextId });
    }
  }, [externalSelectedExtensionId, updateContext]);
  ...
  const cleanupSocket = useCallback(
    (ref, pingRef, shouldNotifyServer = false) => {
      try {
        // Clear ping interval first
        if (pingRef?.current) {
          clearInterval(pingRef.current);
        ...
        if (shouldAutoSend) {
          ...
          if (socket && socket.readyState === WebSocket.OPEN) {
            ...
            (async () => {
              try {
                const selectedExtensionId =
                  context.selectedExtensionId ||
                  (await readExtensionStorage(
                    STORAGE_KEYS.SELECTED_EXTENSION_ID,
                  ));
                const userUuid = context.userUuid || user?.uuid;
                ...
                if (userUuid) {
                  try {
                    const agentsResp =
                      await extensionAgentService.getExtensionAgents({
                        user_uuid: userUuid,
                      });
                    const agentsList = Array.isArray(agentsResp?.data)
                      ? agentsResp.data
                      : Array.isArray(agentsResp)
                        ? agentsResp
                        : [];
                    ...
                    if (selectedExtensionId && agentsList.length > 0) {
                      const matchingAgent = agentsList.find(
                        (agent) => agent.extension_id === selectedExtensionId,
                      );
                      if (
                        matchingAgent &&
                        matchingAgent.conversation_starters
                      ) {
                        conversationStarter =
                          matchingAgent.conversation_starters;
                      }
                    ...
                    console.warn(
                      "[useKnowledgeWorkflow] Failed to fetch conversation starters, using default:",
                      error,
                    );
                  }
                }
                ...
                // Send the conversation starter to socket
                if (socket.readyState === WebSocket.OPEN) {
                  const tokenFromStorage = await readExtensionStorage(
                    STORAGE_KEYS.ACCESS_TOKEN,
                  );
                  const wsToken = normalizeWsToken(
                    tokenFromStorage || accessToken,
                  );

                  // Resolve embedding model settings from storage
                  let currentIsOpenAI = isOpenAIRef.current;
                  let currentModel = EMBEDDING_MODELS.OPENAI;
                  try {
                    const storedModel = await readExtensionStorage(
                    ...
                    if (
                      ...
                      const storedFlag = await readExtensionStorage(
                        STORAGE_KEYS.IS_OPENAI,
                      );
                      if (typeof storedFlag === "boolean") {
                        currentIsOpenAI = storedFlag;
                      }
                ...
                console.error(
                  "[useKnowledgeWorkflow] Error sending auto prompt:",
                  error,
                );
                safeSetState((prev) => ({
                  ...prev,
      ...
      if (isStreamStart || isStreamChunk || isStreamEnd || isStreamFinal) {
        ...
        if (isStreamEnd || isStreamFinal) {
          ...
          const isFirstResponse =
            !stateRef.current?.chat?.hasReceivedFirstResponse;
  ...
  const openChatSocketConnection = useCallback(
    async ({
      ...
      const wsToken = normalizeWsToken(tokenFromStorage || accessToken);
      ...
      const socketUrl =
        ...
        `&access_token=${encodeURIComponent(wsToken)}` +
        `&is_openai=${encodeURIComponent(currentIsOpenAI)}` +
        `&open_ai=${encodeURIComponent(currentIsOpenAI)}` +
        `&embedding_model=${encodeURIComponent(currentModel)}` +
        `&is_faiss=false` +
        `&is_extension=true`;
  ...
  const waitForLlmViaWebSocket = useCallback(
    ({ batchId, taskId, visitorId, runId }) => {
      return new Promise(async (resolve) => {
        ...
        const url = `${scrapperWsBase}/ws/markdown/${encodeURIComponent(
          batchId,
        )}/?token=${encodeURIComponent(wsToken)}&access_token=${encodeURIComponent(wsToken)}`;
        const socket = new WebSocket(url);
        llmSocketRef.current = socket;
    ...
    [accessToken, cleanupSocket, isActive, setStepStatus],
  );
  ...
  const waitForKbViaWebSocket = useCallback(
      ...
      return new Promise(async (resolve, reject) => {
        ...
        socket.onmessage = (event) => {
          ...
          if (code === 200 && msg === "Knowledgebase built") {
            ...
            // Use a simple rotation based on message count
            const messageIndex =
              Math.floor(Date.now() / 2000) % customMessages.length;
            const progressNote = customMessages[messageIndex];
    ...
    [accessToken, cleanupSocket, isActive, safeSetState, setStepStatus],
  );
    ...
    async ({ knowledgeBatchId, visitorId, userUuid, runId }) => {
      ...
      if (hasExistingSession && knowledgeBatchId) {
        try {
          ...
          const sessionsList = Array.isArray(sessionsResp?.data)
            ? sessionsResp.data
            : [];
      ...
      cleanupSocket(chatSocketRef, chatPingRef, true);
      await openChatSocketConnection({
        knowledgeBatchId,
        visitorId,
        userUuid,
        sessionId,
  ...
  const resolveHtmlSelectionPreference = useCallback(async () => {
    ...
    try {
      ...
      console.warn(
        "[useKnowledgeWorkflow] Failed to read cached HTML selection",
        error,
      );
    }

    try {
      const remote = await scrapperService.getHtmlSelectionPreference(
        userUuid,
        {
          accessToken,
          refreshToken,
      ...
      console.warn(
        "[useKnowledgeWorkflow] Failed to fetch HTML selection preference",
        error,
      );
    }
    ...
    async ({ title, note, url }) => {
      ...
      try {
        ...
        if (!uploadFile) {
          ...
          try {
            const tooSmall = !processedHtml || processedHtml.length < 400;
            const looksTemplateOnly = /<body[^>]*>\s*<\/body>/i.test(
              processedHtml,
            );
            if (tooSmall || looksTemplateOnly) {
              console.warn(
        ...
        try {
          ...
          // Confirm successful upload
          const fileType =
            uploadContentType === "application/pdf" ? "PDF" : "HTML";
          setStepStatus("llm", "in-progress", {
            note: `${fileType} uploaded successfully - processing content`,
          });
        } catch (e) {
        ...
        try {
          ...
          try {
            ...
            console.warn(
              "[useKnowledgeWorkflow] Failed to read embedding model from storage",
              error,
            );
          }
          console.log("[useKnowledgeWorkflow] Using embedding model setting", {
  ...
  const handlePanelClose = useCallback(() => {
    ...
    if (
      ...
      console.log(
        "[useKnowledgeWorkflow] Panel closed - closing socket connection",
      );
      cleanupSocket(chatSocketRef, chatPingRef, true);
    }
  }, [cleanupSocket]);
  ...
  const handlePanelOpen = useCallback(async () => {
    ...
    const currentState = stateRef.current;
    const { knowledgeBatchId, sessionId, visitorId, userUuid } =
      currentState?.context ?? {};
    ...
    if (knowledgeBatchId && sessionId && currentState?.status === "ready") {
      ...
      if (!hasActiveSocket) {
        console.log(
          "[useKnowledgeWorkflow] Panel opened - reconnecting socket",
        );
        try {
          await connectChatSocket({
            knowledgeBatchId,
          ...
          console.warn(
            "[useKnowledgeWorkflow] Failed to reconnect socket on panel open:",
            error,
          );
        }
      }
